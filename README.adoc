...
.Quelles commandes avez-vous tapées ?
....
git add . && git commit -m "Preparation cargo" && git push
....
...
+
....
&: comme reference en c, on emprunte le contnu mais la propriete reste au meme endroit
mut: mutable, on peut modifier la valeur de la variable
....
.. Quel est le type de retour de `read_line` ? Quel est son utilité ?
+
....
Result, il permet de savoir si la lecture s'est bien passée ou non
....
